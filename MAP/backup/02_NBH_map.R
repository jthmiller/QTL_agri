...
cross <- subset(cross,ind=!cross$pheno$ID==c('NBH_5646','NBH_5528')
################################################################################
...
################################################################################
##png(paste0('~/public_html/NBH_RF_',i,'.png'))
## plotRF(cross, chr=i)
##dev.off()
##png(paste0('~/public_html/NBH_gts_cleaned',i,'.png'),height=2500,width=4500)
...
cross.order <-tspOrder(cross = cross,hamiltonian = TRUE, method="concorde",concorde_path = "/home/jmiller1/concorde/TSP")

cross_map <-  est.map(cross, error.prob=0.01,map.function="kosambi",maxit=1000,
...
cross <- orderMarkers(cross, verbose=FALSE,use.ripple=FALSE, error.prob=0.01,
  sex.sp=FALSE, map.function="kosambi", maxit=10000, tol=1e-3)

rip1r <- ripple(cross, chr=i, window=5, error.prob=0.01, sex.sp=F, maxit=1000, method="countxo")

cross <- switch.order(cross, i, rip1r[2,])
...
#drp1 <- droponemarker(cross, error.prob=0.01,map.function="kosambi",maxit=1000,
#  tol=1e-3, sex.sp=FALSE,verbose=FALSE)
#
#cross <- dropByDropone(cross, drp1, endMarkerThresh = 20, midMarkerThresh = 20,
#  map.function = "kosambi", re.est.map = T, error.prob=0.01, maxit=1000,
#  tol=1e-4, sex.sp=FALSE, verbose=FALSE)

png(paste0('~/public_html/NBH_RF_reordered',i,'.png'))
 plotRF(cross, chr=i)
dev.off()

filename <- paste0('/home/jmiller1/QTL_Map_Raw/ELR_final_map/NBH_gts_CHR',i,'_downsmpl_reordered')
