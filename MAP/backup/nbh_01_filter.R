...
bfix <- intersect(bfix1,bfix2)

gt <- geno.table(cross)
gt.cp <- intersect(rownames(gt[which(gt$P.value > 0.05),]) ,bfix)

rats <- sapply(cross$pheno$ID, function(X){
tb <- table(pull.geno(cross)[cross$pheno$ID==X,gt.cp])
(tb['1'] + tb['3'])/tb['2']
})

hets <- sapply(cross$pheno$ID, function(X){
tb <- table(pull.geno(cross)[cross$pheno$ID==X,gt.cp])
tb['2']/sum(tb)
})

png(paste0('~/public_html/NBH_rats',i,'.png'))
plot(sort(hets))
dev.off()

par.rats <- sapply(cross$pheno$ID, function(X){
tb <- table(pull.geno(cross)[cross$pheno$ID==X,gt.cp])
tb['1']/sum(tb,na.rm=T)
})

png(paste0('~/public_html/NBH_rats_is_NBH',i,'.png'))
plot(sort(par.rats))
dev.off()

par.rats.bi <- sapply(cross$pheno$ID, function(X){
tb <- table(pull.geno(cross)[cross$pheno$ID==X,gt.cp])
tb['3']/sum(tb,na.rm=T)
})

png(paste0('~/public_html/NBH_rats_is_BI',i,'.png'))
plot(sort(par.rats.bi))
dev.off()
