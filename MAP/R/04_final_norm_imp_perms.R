...
perms <- scantwo(gg_step2, incl.markers=F, chr = c(1:4,6:24), pheno.col=5, model="normal", method="imp", clean.output=T, clean.nmar=10,clean.distance=10,n.perm=perm_count, assumeCondIndep=T, n.cluster=22)
...
norm.add <- stepwiseqtl(gg_step2, incl.markers=F, additive.only = T, model='normal', method = "imp", pheno.col = 5, scan.pairs = F, max.qtl=8)
...
full.norm.imp <- stepwiseqtl(gg_step2, penalties=pens, incl.markers=F, additive.only = F, model='normal', method = "imp", pheno.col = 5, scan.pairs = T, max.qtl=8, chr=qtls_chr, qtl=norm.add.qtls)
...
full.norm.hk <- stepwiseqtl(gg_step2, penalties=pens, incl.markers=F, additive.only = F, model='normal', method = "hk", pheno.col = 5, scan.pairs = T, max.qtl=8, chr=qtls_chr, qtl=norm.add.qtls)
