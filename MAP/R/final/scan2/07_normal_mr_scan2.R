...
norm.mr.2.cov <- scantwo(cross, pheno.col=5, model="normal", method="mr",
 clean.output=T, clean.nmar=10, clean.distance=10, maxit=1000,
 assumeCondIndep=T, n.cluster=cores, use="complete.obs", addcovar=g)

norm.mr.2 <- scantwo(cross, pheno.col=5, model="normal", method="mr",
 clean.output=T, clean.nmar=10, clean.distance=10, maxit=1000,
 assumeCondIndep=T, n.cluster=cores, use="complete.obs")

################################################################################
save.image(file.path(mpath,paste0(pop,'_scan2_normal_mr.rsave')))
################################################################################
