...
bin.imp.perms.2 <- scantwo(cross, pheno.col=4, model="binary", method="imp",
...
bin.imp.perms.pens <- calc.penalties(bin.imp.perms.2, alpha=0.1)

bin.imp.perms.1 <- scanone(cross, pheno.col=4, model='binary', method = "imp",
 n.perm = 10, n.cluster=cores)

lod <- summary(bin.imp.perms.1)[1]

assign(paste0("bin.imp.perms.2.",arraynum), bin.imp.perms.2)
assign(paste0("bin.imp.perms.1.",arraynum), bin.imp.perms.1)

summary(bin.imp.perms.2)
summary(bin.imp.perms.pens)
print(bin.imp.perms.pens)
print(paste('done with', perm_count, 'scan 2 permutations'))
print(summary(bin.imp.perms.1))

################################################################################
save.image(file.path(mpath,paste0(pop,arraynum,'_scan_perms_bin_imp.rsave')))
################################################################################
