...
norm.em.perms.2 <- scantwo(cross, pheno.col=4, model="bin", method="em",
 incl.markers=F, clean.output=T, clean.nmar=15, clean.distance=20,
...
norm.em.perms.1 <- scanone(cross, pheno.col=5, model='normal', method = "em",
 n.perm = 10000, n.cluster=cores)

lod <- summary(norm.em.perms.1)[1]

summary(norm.em.perms.2)
summary(norm.em.perms.pens)
print(norm.em.perms.pens)
print(paste('done with', perm_count, 'scan 2 permutations'))
print(summary(norm.em.perms.1))
