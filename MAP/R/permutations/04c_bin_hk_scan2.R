...
if(pop == 'ELR'){
 sone.perms <- scanone(subset(cross, chr=c(1:4,'X',6:17,19:24)), pheno.col=4, model="binary", method="hk", n.perm=1, n.cluster = cores, perm.Xsp=T)
} else {
 sone.perms <- scanone(subset(cross, chr=c(1,3,4,'X',6:24)), pheno.col=4, model="binary", method="hk", n.perm=1, n.cluster = cores, perm.Xsp=T)
}
...
bin.hk.2 <- scantwo(cross, pheno.col=4, model="binary", method="hk",
 incl.markers=F, clean.output=T, clean.nmar=25, clean.distance=25, maxit=100,
