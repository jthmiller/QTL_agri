...
file_list <- list.files(mpath, 'NEW_all_mark_?[0-9]?[0-9]_tsp.csv')

chr <- gsub("NEW_all_mark_",'',file_list)
chr <- as.numeric(gsub("_tsp.csv",'',chr))

NEW <- lapply(file_list,function(X){ read.cross(file=X,format = "csv", dir=mpath, genotypes=c("AA","AB","BB"), alleles=c("A","B"),estimate.map = FALSE)})

gnos <- lapply(NEW,function(X){
  data.frame(X[[1]][[1]][['data']],stringsAsFactors=F)
})
gnos <- do.call(cbind,gnos)
gnos <- cbind(NEW[[1]]$pheno,gnos)
gnos$ID <- as.character(gnos$ID)

m_names <- unlist(sapply(NEW,function(X){
  markernames(X)
}))

colnames(gnos) <- c('Pheno','sex','ID','bin','pheno_norm',m_names)
rownames(gnos) <- NEW[[1]]$pheno$ID

map <- c(colnames(NEW[[1]]$pheno),unname(unlist(sapply(NEW,pull.map))))
chr <- c(colnames(NEW[[1]]$pheno),gsub(":.*","",m_names))
info <- c(colnames(NEW[[1]]$pheno),m_names)
...
write.table(to_write,file.path(mpath,'NEW.mapped.tsp.csv'),sep=',',row.names=F,quote=F,col.names = F)
...
####################################################################################
save.image(file.path(mpath,'single_scans.new.rsave'))
################################################################################
